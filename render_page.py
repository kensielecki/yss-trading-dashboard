    ...
    for date in session_dates:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Trailing 10-day VWAP" if first_rolling_trace else None,
                showlegend=first_rolling_trace,
                line=dict(color="rgba(180, 50, 50, 0.45)", width=2, dash="dot"),
                hovertemplate=(
                    f"%{{x|%b %d}}<br>Trailing 10-day VWAP at close: ${float(vwap):.2f}<extra></extra>"
                ),
            ),
            secondary_y=False,
        )
...
<body>
  ...
  <div class="chart-wrap">
    ...
    <p class="chart-footnote">Green dots: session opens &nbsp;|&nbsp; Red dots: session closes &nbsp;|&nbsp; Dotted red segments: trailing 10-day VWAP as of each session close &nbsp;|&nbsp; Orange line: current 10-day VWAP.</p>
  </div>
